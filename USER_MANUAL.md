...
If you already have a manuscript written outside QuillWork (Word, Scrivener, plain text, whatever — as long as you can get it into a `.txt` or `.md` file or onto the clipboard), QuillWork can ingest the whole thing and build the bible automatically.

1. Click **Projects → New project** first (or start from a blank project) — Import works on the current project.
2. Click **Import manuscript** in the sidebar.
3. Either:
...
5. QuillWork runs through several stages automatically, shown live as they complete:
   - **Chapter splitting** — detects chapter breaks (using markers like "Chapter 1" if present, falling back to AI-assisted detection if not)
...
**Notes:**
- Import can take a while for a long novel — it's making many AI calls. Leave the tab open until it reports "Import complete!"
...
3. Click **Link & sync**. QuillWork reads the file, matches what it finds to your project's existing chapters by title, and merges in whatever's new or changed — exactly like a Scrivener sync, including never deleting a chapter automatically if it's gone missing from the document (you'll just see it flagged).
...
| **Bottom bar** | Formatting tools (new paragraph, headings, table, embed image, source view, bold, italic, underline, strikethrough, colour), quick actions (Check, Refine, Mark as claim), a free-text append box, and the Scene beat and Chat panels (which expand upward when opened) |
...
4. Click **Set up now**. The models download through Ollama itself, with a progress bar, and QuillWork then fills in your Model, Fast model and Vision model settings for you.
...
Model choice matters a lot for prose quality — this is a local generation tool, so the model you load *is* the writer. QuillWork's prompt engineering (see [Section 16](#16-reference--anti-ai-prose-rules)) pushes any model toward more human prose, but a stronger base model will always sound better.
...
**Character psychology.** Click **Suggest character psychology (AI)** to have QuillWork infer a **Core want**, **Core fear**, **Defense mechanism** and **Wound** from what the character actually does on the page. It uses the same chain of choices, changes and discoveries as the arc simulator, and lists the chapters the inference came from ("Inferred from chapters ..."). It is a suggestion to read, not something that overwrites your Personality, Backstory or Arc, and **Refresh** re-runs it once the character has more history. It needs the Story Analyst to have analysed enough of the character; otherwise a message says their real choices and changes aren't tracked yet.
...
2. Every fact is fed into every AI writing call by default, so the model is nudged to stay consistent with it without you having to repeat it in every brief. Past a couple of dozen facts, some AI tools switch to sending only the ones relevant to the current passage rather than the whole list. See [9.3](#93-where-retrieval-kicks-in) for exactly which tools and why.
...
Every AI writing call in QuillWork bakes in the [anti-AI-prose rules](#16-reference--anti-ai-prose-rules) — you don't need to ask for "less robotic" prose, it's the default.

### 8.1 Scene beat

The main way to generate new prose. Click **Scene beat** at the bottom of the screen (only available with a chapter open).
...
3. Click **Generate**. The prose streams into an amber-bordered **draft block** directly in your editor, in place, right where your cursor left off.
4. The draft is **fully editable** — click into it and revise before deciding what to do with it.
5. **Stop** halts a generation and keeps what has arrived. **Approve ✓** commits the draft as permanent paragraphs in your chapter. **Regenerate** runs the same brief again for another version without retyping it. **Discard** removes the draft with no trace.
...
Press **Ctrl+Enter** in the brief box to generate without reaching for the button. Beside Generate is a small model chip (labelled "default"): choose another model to use for just this generation. It is not saved and resets when you reload. Refine has the same chip. If you turn on **Check continuity when I approve a Scene beat draft** in Settings ([Section 4](#two-more-settings-switches)), a small card also runs a continuity check on the passage you just approved, without blocking you.
...
**Pay off an overdue promise.** Choose this type and a second picker lists every open question or promise your manuscript has raised, ranked by how overdue it is (how long it has been open, weighted by how central it is to the story). Pick one, and the three ideas aim specifically at resolving it. The list fills from the [Story Analyst](#812-story-analyst); until you've run it, the picker says no open questions were found yet.

**Future consequences of a chapter's events.** Pick a chapter, and QuillWork tells the model the real events in it and what the causality graph already shows following from them, then asks for three new consequences that have not been written yet. It is the mirror of the Domino test: instead of "what breaks if I cut this?", it asks "what should follow from this?".
...
Every path is checked against the bible so it stays plausible — no path will have a character act against their established personality or use knowledge they don't have.
...
2. Click it. A progress bar shows percentage complete and an elapsed/estimated-remaining time.
...
4. Click **Apply selected changes**. Only checked items are applied; the chapter is updated and autosaved.
...
3. The continuity report appears below — review it for anything the new chapter contradicts.
...
Type a question and press **Enter** (Shift+Enter for a new line) or click the send button. The conversation stays on screen while the panel is open so you can ask follow-ups, but it is **not saved** — closing the panel or reloading QuillWork clears it. This is a lookup tool, not a place to draft prose.
...
**Six tabs**, switched along the top of the panel:
...
- 🟢 **Working** — an embedding model is reachable, ready to go
- 🟡 **No embedding model detected** — click **Set up embedding model automatically**. QuillWork checks for one already available and, if none exists, downloads a small one (`nomic-embed-text`, ~270MB) and configures it — after confirming with you first, since it's a network download.
...
- Every time you save a chapter (manual save, autosave, or switching chapters), it's automatically re-embedded in the background **only if the text actually changed** — no wasted work.
- The same applies to characters, locations, facts, worldbuilding, relationships, and Research Workspace uploads: adding, editing, or deleting one queues a background re-embed, again only for what actually changed.
- **Embed manuscript** (sidebar, Story section) forces a full rebuild across everything above, chapters, bible entries, and research assets alike. Use this after a bulk import from an older QuillWork version, after upgrading from a version that predates this section, or if you've made large manual edits outside the normal save flow.
- The **Search index** panel shows how many chapters/passages, bible entities, and research assets are currently indexed, and lets you rebuild manually at any time.
- Upgrading an existing project to the newer storage format triggers a one-time automatic re-embed of everything already in it. There's nothing to do on your end, and retrieval simply falls back to a plain, unscoped bible view until it finishes (the same graceful fallback Chat already uses when no embedding model is set up at all).
...
Scene beat, Refine, and Chat go further: once you're tracking more than a handful of facts or worldbuilding entries, these three scope down to just the ones actually relevant to the passage at hand, rather than sending your whole list on every single call (see [6.11](#611-facts--constants)). Nothing is deleted or hidden from your bible by this, the rest are simply left out of that one prompt; a character actually present in the scene, and their relationships, are always included regardless of how well they happen to match the search. Continuity check keeps the full bible in view instead, since cross-referencing everything at once is its whole job, and adds retrieval on top for earlier chapters and relevant [Research Workspace](#612-research-workspace) references.
...
| **About the author** | A back-matter author bio page |
...
- **Restore** — click **Restore** to roll the whole novel back to that snapshot. Before it does anything, QuillWork takes a **safety snapshot of your current state** and puts it at the top of the list — so a restore is itself undoable; you can never lose your place by going back.
...
3. Closes QuillWork and puts the new files in, keeping a copy of every file it replaced. Your projects, Bible, settings and licence are separate from the program files and are never touched.
4. Starts the new version and waits for it to answer. This window then reloads by itself.

**If something goes wrong.** A failure before step 3 leaves everything exactly as it was, and the window says what happened. If the new version is put in but does not start, QuillWork puts the previous files back and starts the old version again, then tells you the update did not work and why. Nothing is lost either way. QuillWork will not start an update while an AI job is running: let the job finish, or press **Stop AI**, then try again.
...
These rules are injected into every prose-generation and prose-review call in QuillWork. They're the baseline for how QuillWork asks the model to write.
...
| Character psychology | Infers core want, core fear, defence mechanism and wound from what the character actually does on the page, with the chapters cited. | Characters > open a character > Suggest character psychology (AI) |
...
Open it from `Sidebar > AI Tools > Story Analyst`, or press **Ctrl+K** and type "Story Analyst". It has six tabs: Events & causality, Questions & promises (Narrative Debt), Known facts, Health report, What if…, Patterns. Run **Scenes** analysis on a chapter first (A.2); the Analyst reads scenes. Every result is labelled **Deterministic** (calculated straight from your story data, identical on every run) or **Judged** (produced by the AI, always with the quote it rests on).
...
| Show in chapter | Closes the panel, opens the chapter and selects the exact passage a finding, event, question or motif rests on. | The "Show in chapter" button on any quoted evidence |
...
| Questions and promises | Every question or promise the text raises, with centrality and a status you set (open, partly answered, answered, left open on purpose, red herring, abandoned). | Story Analyst > Questions & promises (Narrative Debt) |
...
| Quick AI setup | Optional guided setup. Opens Ollama's official download page, waits for Ollama to respond, recommends a model tier for your hardware, then downloads your writing, vision and fast models through Ollama. Skippable. | Settings > Quick AI setup; or the link in the first-run wizard's "Connect your AI model" step |
...
| Linked Scrivener sync | Checks a linked Scrivener project every 30 seconds and merges in only the chapters that changed. A chapter removed in Scrivener is flagged, never deleted. | Sidebar > Import from other tools > Scrivener. **Unlink** stops it. |
| Linked Word or ODT sync | Checks a linked .docx or .odt file every 30 seconds and merges in changed chapters, matched by title. | Sidebar > Import from other tools > Word/ODT sync. **Unlink** stops it. |
...
| Interrupted analysis notice | If a Story Analyst run or health report was cut off (for example the app closed), opening the Story Analyst shows a banner offering to resume it. Finished steps are not redone. | Sidebar > Story Analyst. |
