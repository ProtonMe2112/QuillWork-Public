...
*Version: Beta (pre-release). This manual is kept up to date alongside the app and is what QuillWork's own **Help** shows (top bar > Help, or **Ctrl+K** then "User manual"). It has a search box of its own at the top — type a word and it filters to matching sections, highlights every occurrence, and jumps straight there. [Appendix A](#appendix-a-complete-function-reference) at the end lists every function, what it does and exactly how to reach it.*
...
1. **Launch QuillWork.** The installer adds a **QuillWork** icon — to your Desktop and Start Menu on Windows, to your Desktop and applications menu on Linux — use either. Without it, double-click `QuillWork.vbs` (silent, recommended) or `Start QuillWork.bat` (shows a console window — useful if something goes wrong) on Windows, or `QuillWork.desktop` on Linux, in the folder you installed to. Your browser opens automatically to `http://localhost:5000`.
2. **Start LM Studio or Ollama** and load a model, if you haven't already (see [Section 4](#4-connecting-an-ai-model)).
3. Click **Projects** in the top bar → **New project**. Give it a title and pick a **project type**: Novel (the default), Short story, Non-fiction, Historical fiction, Screenplay planning, or Series planning. A type is a preconfigured starting point, not a separate mode — Non-fiction relabels Chapters to Sections, Characters to People, and Plot threads to Open questions; Screenplay planning relabels Chapters to Scenes; Historical fiction surfaces the Research Workspace's source tiers ([6.12](#612-research-workspace)) from the start; Series planning starts a new series ([Section 7](#7-series--sharing-a-world-across-multiple-books)) alongside this book so its characters, locations and worldbuilding can be shared across every book in it. Every type still writes ordinary prose chapters underneath — a Screenplay project plans scene by scene, but QuillWork doesn't produce industry script format (sluglines, action/dialogue columns); format the actual script elsewhere once you're happy with the plan. Add a genre too if you like (optional, and just informs the AI's tone — it doesn't restrict anything). If your genre matches one QuillWork recognizes (Fantasy, Science Fiction, Mystery/Thriller, Romance, Horror, Historical, Literary, Young Adult), World & style starts with a short list of genre-relevant prompts already in world-building notes, edit or delete them freely, they're just a starting scaffold, never fake content — a Short story, Non-fiction or Screenplay planning project starts with its own equivalent scaffold instead, whether or not the genre matches anything.
...
**Not sure which import you need?** Click **Import** in the top bar, next to Export, or **Bring in your writing** at the top of the sidebar (or press **Ctrl+K** and type "Bring in"; the first-run guide and the link at the top of the Import manuscript panel open the same window). It asks **What are you bringing?** and shows one line for each choice saying what it will and will not bring:
...
- **If a chunk fails** (most often a timeout on a long manuscript or a slow model): everything else keeps going, and the import panel stays open afterward with a **Retry failed chunks** button reporting exactly how many failed, so you can try just those again without re-running the whole import. A failure with the combined-extraction option above retries as one call, same as it ran the first time. A chapter whose own summary, title or pacing failed to generate is retryable the same way, alongside any other failed chunk.
...
**If a chunk of that "other project content" fails** (most often a timeout): everything else keeps going, and QuillWork reports exactly how many chunks failed afterward, with a **Retry failed chunks** button to try just those again.
...
| Region | Contents |
|---|---|
| **Top bar** | The **QuillWork** name itself (with the version it's running, and click it for version, computer, memory and when this copy was installed or updated), project name, save status, **Autosave** switch, the theme button, a **Ctrl K** command-palette button, **Save**, **Import**, **Export**, **Projects**, **Beta report**, **Settings**, **Help** (opens this manual inside QuillWork), and the **Close** button that shuts the server down cleanly |
...
**Home.** The first destination in the sidebar, and where QuillWork opens by default: your project's title and total word count, **Continue writing** (or **Start writing** on a brand-new project with nothing in it yet, which creates and opens your first chapter directly), your own writing goals, Narrative Debt, outstanding review tasks, the Story Analyst's last-run status, your most recently touched chapters and most recently added research, and your other projects. **Writing goals** are two optional targets you set yourself, right there on Home: a daily word count (a personal habit target, the same regardless of which project is open) and a target chapter count for the current novel -- leave either blank for no goal, and a progress bar appears under whichever one you've set. Today's word count only ever counts real growth; a cut or a shorter rewrite is never subtracted from it. **Review tasks** is the same count of pending findings and Bible-update suggestions the [AI Activity screen](#133-ai-activity) tracks; click it to open that panel directly. **Recent chapters** and **recent research** each open straight into the real thing -- the editor, or the research item itself.
...
Two things worth knowing. Automatic setup needs the Ollama backend; with LM Studio you choose and load models yourself, as described below. And QuillWork never hosts or proxies a model file: every download comes from Ollama's own library, through your own Ollama. QuillWork checks for an updated recommendations list over the same channel Check for updates already uses, so the picks can change as better models appear without a new release; your bundled copy is the fallback whenever that check is off, fails, or you are offline, so this never adds a wait or a new way for setup to fail.
...
Click **Segment this chapter** to segment just the open chapter, or **Segment whole manuscript** to run every chapter at once (a first run on a long novel can take a while, since it's genuinely re-reading each chapter — a progress log shows which chapter is currently being processed). These are deliberately named differently from Story Analyst's own **Analyse this chapter** (below): this button only splits the chapter into scenes and reads each one's POV, summary and dialogue, it does not find events, causality or facts. You rarely need to press it by hand -- Story Analyst re-segments a chapter itself, automatically, the moment it needs to -- so use this panel on its own only to preview scene boundaries or fix one by hand (lock, split, merge). Boundaries are detected from explicit scene-break markers (`***`, `# # #`, and similar) and headings, plus a blank-line gap combined with a clear time or location shift — a chapter with no detected break just becomes one scene, which is normal, not a failure. A line of nothing but break markers (or two in a row, or one straight after a heading) is only ever a break, never a scene of its own. Running analysis again leaves every scene whose text has not changed exactly as it was, along with everything the Story Analyst has worked out about it, so after an edit only the scenes you actually changed are read again.
...
**Story Overview**, at the top of the Plan section in the sidebar, assembles the whole story onto one screen: your synopsis, the Health Dashboard's own "what to work on next" list, every chapter with its word count and pacing, every character with their role and arc, every plot thread, every open narrative question, your most recent timeline events, and your most recently added research, each with a link or button to see more. Nothing here is computed specially for this screen -- every figure is read from the same place its own panel already shows it, so nothing can disagree between the two.

Click a chapter and it opens straight into the editor (closing Story Overview first, since you're switching what you're looking at). Click a character or a plot thread and its own floating window opens on top, exactly as if you'd opened it from the Bible menu -- Story Overview stays visible behind it, so you can glance back at the rest of the story while you edit. A narrative question's **Show in chapter** button takes you straight to the passage that raised it. Research and the "See full dashboard"/"Open Visual Timeline" links open their own full panel.
...
- **Paste from clipboard**: one click, no window to fill in and no topic to choose first. QuillWork reads whatever is actually on your clipboard: an image becomes an upload, a web address becomes a web reference (fetched the same way **Add web reference** does, below), and anything else becomes a note holding that text. A confirmation appears once it is saved; open the new card afterward to add a caption, topics or tags.
...
- **Add web reference**: a URL, plus your own notes if you want them. QuillWork tries to fetch the page for its real title, author, publish date and body text, and its own preview image for the card thumbnail (where one exists); a title you type yourself is always kept over a fetched one, and a page that can't be reached (offline, blocked, not HTML) just falls back to saving the URL alone, same as if nothing had been fetched.

**Source, Author and Date.** Every item's own window (not just web references) has an **Author** and a **Date** field alongside its Caption, for citing where something actually came from instead of folding that into free text. Both are optional and stay blank until you (or a successful web fetch) fill them in.

**Provenance.** Every item's own window has a **Provenance** field with four states: **Verified source**, **Your interpretation**, **Fictional invention**, and **Unresolved question**. A new item starts **Unset**, and Unset always shows as its own visible badge on the card, so research is never silently treated as established fact just because you never got around to marking it. Below Provenance, an existing item also has **Facts from this source**: short statements you note as derived from it, each with its own tier, in case a source you have marked Verified overall still has one detail you are unsure of. The workspace's provenance filter (next to the kind filter) finds every item resting on a given tier, whether that tier is the item's own or belongs to one of its facts, so you can pull up everything still unresolved in one list. Most useful in historical or research-heavy fiction, where what you found, what you are guessing, and what you made up need to stay visibly separate.
...
**Eight tabs**, switched along the top of the panel:
...
One screen answering "what should I work on next?" **Narrative Debt** sits at the top as the headline figure, exactly as it does in Patterns (it is the same number). Above the six areas below it, **What to work on next** shows a short priority list: at most one item per area, its own worst one, so you see a spread across the whole manuscript rather than a pile of everything wrong with a single scene. Below that, six collapsible areas, each with its own count: **Plot** (dead-weight scenes), **Character development** (characters whose importance is fading, and characters with real presence who never initiate anything), **Narrative questions** (promises and questions open well past a reasonable length), **Pacing** (long stretches with no state change), **Continuity** (results whose quote no longer matches the text as written, and results a support check said their quote does not fully show), and **Structure** (chapters nothing downstream depends on, runs of reveals that tighten toward the end, and a revelation with no earlier suspense or irony built around it). Every item has a **Show in chapter** or **Open chapter** button. Nothing on this screen is a fresh reading: every figure is read straight from what Patterns, the overdue-promises ledger, the Evidence check and the Health report's own accelerating-reveals and suspense notes already work out, so the Dashboard can never disagree with them, and opening it never starts a model call. An area with nothing flagged says so plainly rather than being left blank.
...
- **Motifs, symbols & themes** (Calculated finding, AI-judged development): track a recurring image, object or idea by typing the words to look for (several allowed, separated by commas, for example "lighthouse, beacon") and choosing Motif, Symbol or Theme. QuillWork finds every scene that mentions it, lists the passages (each with **Show in chapter**), and plots which chapters it appears in. The Health report then judges how it develops across the book. QuillWork only observes these; it never writes one into your prose.
  - **Seed from selected passage**: select text in your chapter, then click this to have QuillWork propose the tracking words for you, grounded in that passage, ready to review before you click Track.
  - **Discover themes (AI)**: groups scenes that share MEANING, not shared words, using the same scene comparisons Emotional echoes and Parallel scenes already make, and proposes a name and tracking vocabulary for each recurring group it finds, with the scenes behind it. Few candidates at a time. **Track this** creates the theme motif and searches for it immediately, the same as typing it in by hand; **Dismiss** is permanent; a later scan never proposes the exact same group of scenes again.
...
- **New project** — starts a blank bible with a title, a project type (Section 1, step 3), and a genre.
...
**A safety snapshot, taken automatically.** Right before Update now proceeds, QuillWork takes a version-history snapshot of the project you currently have open — the same mechanism already used before every import — so there is a real restore point from just before the update, not just the program files being put back. Find it afterward in that project's own **Versions** panel if you ever need it.

**Rolling back to the previous version, on purpose.** QuillWork keeps the files from your last version for a while after a successful update, not just after a failed one. If a new version misbehaves in some way its own health check didn't catch, open **Settings → Updates** — a **Roll back to vX.X.X** button appears whenever a previous version is available to go back to. It works the same way as Update now in reverse: saves your open chapter, closes QuillWork, restores the previous version's files, and starts it back up, confirming it actually answers before calling it done. A safety snapshot of your current project is taken first, the same as for a forward update. Once you roll back, that backup is used up — a fresh one is kept the next time you update.

**Trying an update before trusting it with production.** Once an update is downloaded and ready, the window offers a third choice alongside Cancel and Update now: **Try it first**. This runs the new version on its own standing copy — its own program files, its own port, and its own copy of your current data — without touching your real projects at all. A banner appears with three actions:

- **Open validation copy** opens it in a new browser tab (a different address from your usual one) so you can actually use it: open a project, try Story Analyst, whatever you want to check.
- **Promote to production** applies the same update to your real install for real, the same as Update now would have, then stops the validation copy. Your real data was never touched by the validation copy, so nothing from it needs to be brought across.
- **Discard** just stops the validation copy. Your production install was never touched either way, and stays on its current version.

The validation copy's data is a fresh copy of your production data every time you use Try it first, never something that builds up on its own — anything you do inside the validation copy itself is thrown away the moment you Promote or Discard. Think of it as a safe place to kick the tyres, not a second real workspace.
...
|---|---|---|
| Home | Title, word count, Start/Continue writing, your own daily word and chapter goals with progress bars, Narrative Debt, outstanding review tasks, the Story Analyst's last-run status, recent chapters, recent research, and your other projects. | Sidebar > Home |
...
| User manual | Opens this manual inside QuillWork, with its own search box at the top (filters to matching sections, highlights every hit). Error messages deep-link into the relevant section. | Top bar > Help; or Ctrl+K, "User manual" |
...
| Scenes panel | Splits a chapter into scenes and attributes dialogue lines to characters. Per scene: lock, split, merge with next, and once analysed its viewpoint, place, day, mood, summary and functions. Story Analyst does this itself automatically when it runs; use this panel on its own to preview or fix scene boundaries by hand. | Chapter header > Scenes > **Segment this chapter** or **Segment whole manuscript** |
...
| Function | What it does | How to access it |
|---|---|---|
| Story Overview | One screen assembling the synopsis, Health Dashboard priority list, chapters, characters, plot threads, open narrative questions, recent timeline events and recent research. Each card opens the real detail behind it; nothing here is computed specially for this screen. | Sidebar > Plan > Story Overview |
...
| Seed motif from selection | Select a passage in the chapter, then generate its tracking vocabulary from that specific text instead of typing it yourself. | Patterns > Motifs > Seed from selected passage |
| Discover themes (AI) | Groups scenes that share meaning, not shared words, and proposes a name and vocabulary for each recurring group, with the scenes behind it. Track this creates the motif and searches immediately; Dismiss is permanent. | Patterns > Motifs > Discover themes (AI) |
...
| Try it first (validation copy) | Runs a staged update on its own standing copy, its own port, and a fresh copy of your data, without touching production. Promote applies it to production for real; Discard just stops it. Either way production is untouched until you choose Promote. | The update window, once a download is ready > Try it first |
